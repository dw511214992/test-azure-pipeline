...
async function renameBranch(repo: string) {
    // list prs
    ...
    const allPrs: any[] = allPrsResponse.data;
    const prsCreatedByOpenApiAutomation = allPrs.filter(pr => pr.user?.login === 'openapi-sdkautomation[bot]');
